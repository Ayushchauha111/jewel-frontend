...
const TypeBox = ({
  ...
  const reset = (
    ...
    setStatus("waiting");
    // If test.passage is present, use it to set the wordsDict
    if (test && test.passage) {
      const passageWords = test.passage.split(" ");
      const passageWordsDict = passageWords.map((word, index) => ({
        key: index,
        val: word,
      }));
      setWordsDict(passageWordsDict);
      setCountDownConstant(test.duration);
      setCountDown(test.duration);
    } else {
      // Otherwise, generate new words based on the language and difficulty
      if (!isRedo) {
        if (language === CHINESE_MODE) {
          setWordsDict(
            chineseWordsGenerator(
      ...
      let intervalId = setInterval(() => {
        setCountDown((prevCountdown) => {
          if (prevCountdown === 0) {
            ...
            // Process any remaining input before finishing
  ...
  const UpdateInput = (e) => {
    ...
    // Update input words history with proper state management
    setInputWordsHistory(prevHistory => ({
      ...prevHistory,
      [currWordIndex]: newValue.trim()
    }));
  };
  ...
  const checkPrev = () => {
    ...
    if (isCorrect) {
      ...
      setPrevInput(prev => prev + " " + currInputWithoutSpaces);
      return false;
    }
  };
  ...
  // Initial reset on component mount
  useEffect(() => {
    reset(countDownConstant, difficulty, language, numberAddOn, symbolAddOn, false);
  }, []); // Empty dependency array ensures this runs only once on mount

  // Prevent resetting after completing test.passage
  useEffect(() => {
    if (status === "finished" && test && test.passage) {
      console.log(test)
      return;
    }
  }, [status, test]);
