...
const SentenceBox = ({
  ...
  const [sentencesDict, setSentencesDict] = useState(() => {
    return sentencesGenerator(sentencesCountConstant, language,test);
  });
  // enable menu
  ...
  const checkAndUpdateStats = (currSentence, currInput) => {
    const newStats = { correct: 0, incorrect: 0, extra: 0 };
    
    // Compare characters up to the length of the current sentence
    ...
    setStats(prevStats => ({
      correct: prevStats.correct + newStats.correct,
      incorrect: prevStats.incorrect + newStats.incorrect,
      extra: prevStats.extra + newStats.extra,
    }));
  };
