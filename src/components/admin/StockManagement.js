...
function StockManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let imageUrls = formData.imageUrls && formData.imageUrls.length >= MIN_PRODUCT_PHOTOS ? [...formData.imageUrls] : null;

      // Upload 3-5 product photos if new files selected
      if (productPhotoFiles.length >= MIN_PRODUCT_PHOTOS && productPhotoFiles.length <= MAX_PRODUCT_PHOTOS) {
        const formDataUpload = new FormData();
        productPhotoFiles.forEach((file) => formDataUpload.append('files', file));
        const uploadResponse = await axios.post(`${API_URL}/stock/upload-images`, formDataUpload, {
          headers: {
            ...getAuthHeaders(),
  ...
  const handleEdit = (item) => {
    const urls = item.imageUrls && item.imageUrls.length > 0 ? item.imageUrls : (item.imageUrl ? [item.imageUrl] : []);
    setFormData({
  ...
  const handleProductPhotosSelect = (e) => {
    ...
    if (files.length < MIN_PRODUCT_PHOTOS || files.length > MAX_PRODUCT_PHOTOS) {
      setError(`Please select between ${MIN_PRODUCT_PHOTOS} and ${MAX_PRODUCT_PHOTOS} photos (selected: ${files.length}).`);
      setTimeout(() => setError(null), 5000);
      e.target.value = '';
      return;
    }
  ...
  const removeProductPhoto = (index) => {
    if (productPhotoFiles.length > 0) {
      setProductPhotoFiles((prev) => {
        ...
        if (next.length > 0 && next.length < MIN_PRODUCT_PHOTOS) {
          setError(`Product must have between ${MIN_PRODUCT_PHOTOS} and ${MAX_PRODUCT_PHOTOS} photos.`);
          setTimeout(() => setError(null), 4000);
        }
        return next;
      });
    } else if (formData.imageUrls && formData.imageUrls.length > index) {
      const next = formData.imageUrls.filter((_, i) => i !== index);
      setFormData((prev) => ({ ...prev, imageUrls: next }));
      if (next.length > 0 && next.length < MIN_PRODUCT_PHOTOS) {
        setError(`Product must have between ${MIN_PRODUCT_PHOTOS} and ${MAX_PRODUCT_PHOTOS} photos, or leave empty.`);
        setTimeout(() => setError(null), 4000);
      }
    }
  };

  const productPhotoPreviews = productPhotoFiles.length > 0
    ? productPhotoFiles.map((f) => ({ type: 'file', file: f, url: URL.createObjectURL(f) }))
    : (formData.imageUrls && formData.imageUrls.length >= MIN_PRODUCT_PHOTOS
        ? formData.imageUrls.map((url) => ({ type: 'url', url: url.startsWith('http') ? url : `${window.location.protocol}//${window.location.hostname}:8000${url}` }))
        : []);
            ...
            <div className="stock-modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="stock-image-section">
                <label style={{ fontWeight: 600, color: '#34495e', marginBottom: '0.5rem', display: 'block', fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  Product photos (3 to 5)
                </label>
                <p style={{ fontSize: '0.8rem', color: '#7f8c8d', marginBottom: '0.75rem' }}>
                  Upload between 3 and 5 images for this product. First image is used as the main thumbnail.
                </p>
                <input
                  type="file"
                  accept="image/*"
                  multiple
                  onChange={handleProductPhotosSelect}
                  style={{ display: 'none' }}
                  id="product-photos-upload"
                />
                <label htmlFor="product-photos-upload" className="stock-action-btn" style={{ cursor: 'pointer', display: 'inline-block', margin: 0 }}>
                  📷 Select 3–5 photos
                </label>
                {(productPhotoFiles.length > 0 || (formData.imageUrls && formData.imageUrls.length >= MIN_PRODUCT_PHOTOS)) && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '1rem' }}>
                    {productPhotoPreviews.map((p, index) => (
                      <div key={index} style={{ position: 'relative', flex: '0 0 auto' }}>
                        <img
                          src={p.type === 'file' ? p.url : p.url}
                          alt={`Preview ${index + 1}`}
                        ...
                        <button
                          ...
                          style={{ position: 'absolute', top: -6, right: -6, width: 24, height: 24, padding: 0, fontSize: 12, borderRadius: '50%' }}
                          title="Remove photo"
                        >
                          ×
                        </button>
                ...
                {productPhotoFiles.length > 0 && (
                  <p style={{ fontSize: '0.8rem', color: '#27ae60', marginTop: '0.5rem' }}>
                    {productPhotoFiles.length} photo(s) selected. {productPhotoFiles.length < MIN_PRODUCT_PHOTOS && `Add ${MIN_PRODUCT_PHOTOS - productPhotoFiles.length} more to save.`}
                  </p>
                )}
                <div style={{ marginTop: '1rem' }}>
                  <label style={{ fontSize: '0.85rem', color: '#7f8c8d', marginRight: '0.5rem' }}>Or single image (optional):</label>
                  <input
                    type="file"
                    accept="image/*"
                    ...
                    id="image-upload"
                  />
                  <label htmlFor="image-upload" className="stock-action-btn secondary" style={{ cursor: 'pointer', display: 'inline-block', margin: 0, padding: '0.4rem 0.75rem', fontSize: '0.9rem' }}>
                    📷 One image
                  </label>
                </div>
                {imagePreview && !(productPhotoFiles.length >= MIN_PRODUCT_PHOTOS) && (
                  <div className="stock-image-preview" style={{ marginTop: '0.5rem' }}>
